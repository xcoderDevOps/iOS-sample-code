...
var selectMapViewAddress : SelectMapViewAddress?
...
class SelectMapViewAddress: UIViewController {
    ...
    @IBOutlet weak var txtStartType: LetsTextField!
    @IBOutlet weak var txtEndtType: LetsTextField!
    ...
    @IBOutlet weak var btnSelectDate: LetsButton!
    @IBOutlet weak var btnSelectTime: LetsButton!
   ...
   //MARK: - View life cycle -
    override func viewDidLoad() {
        ...
        selectMapViewAddress = self as SelectMapViewAddress
...
//        }
    ...
    /*
     let arr = countryData.compactMap({"\($0.CountryCode) \($0.Name)"})
    ...
    @IBAction func btnLocationTypeClicked(_ sender: UIButton) {
        let arr = DashboardVC.locationTypeArray.compactMap({$0.Name})
        openDropDown(anchorView: sender, data: arr) { (str, index) in
            if sender.tag == 111 {
                self.txtStartType.text = str
                startAddress.locationType = DashboardVC.locationTypeArray[index]
            } else {
                self.txtEndtType.text = str
                endAddress.locationType = DashboardVC.locationTypeArray[index]
            }
        }
    }
    ...
    @IBAction func btnContinueClicked(_ sender: UIButton) {
        ...
        print(btnSelectDate.titleLabel?.text)
        ...
        else if  btnSelectDate.titleLabel?.text == "DD/MM/YYYY" || btnSelectTime.titleLabel?.text == "HH:MM"
        {
            showAlert("Please select a pickup date and time both")
            return
        }
        ...
        else if  distanceDateTime.distance == ""
        {
            ...
            let controll = MainBoard.instantiateViewController(identifier: "AddExtraAddressDetailVC") as! AddExtraAddressDetailVC
            self.navigationController?.pushViewController(controll, animated: true)
        }
        
    }
    
    @IBAction func btnDateClicked(_ sender: LetsButton) {
        openDatePicker(formate: "dd-MMM-yyyy hh:mm a") { (str) in
            distanceDateTime.PickupDate = str
            let dates = str.components(separatedBy: " ")
            self.btnSelectDate.setTitle(dates.first!, for: UIControl.State.normal)
            self.btnSelectTime.setTitle(str.replacingOccurrences(of: dates.first!, with: "").trim, for: UIControl.State.normal)
        }
    }
    
    
    @IBAction func btnTimeClicked(_ sender: LetsButton) {
        openDatePicker(formate: "dd-MMM-yyyy hh:mm a") { (str) in
            distanceDateTime.PickupDate = str
            let dates = str.components(separatedBy: " ")
            self.btnSelectDate.setTitle(dates.first!, for: UIControl.State.normal)
        ...
        placesClient?.fetchPlace(fromPlaceID: data.id, placeFields: fields, sessionToken: nil, callback: {
          ...
          if let place1 = place1 {
            if isStart {
                ...
                let loc = CLLocation(latitude: place1.coordinate.latitude, longitude: place1.coordinate.longitude)
                loc.fetchCityAndCountry { (con, errir) in
                    if let con = con {
                        startAddress.country = con
                    }
                }
            } else {
...
extension SelectMapViewAddress : UITableViewDelegate, UITableViewDataSource
{
...
extension SelectMapViewAddress : CLLocationManagerDelegate, MKMapViewDelegate
{
...
extension SelectMapViewAddress : UITextViewDelegate
{
