...
extension UIViewController {
    ...
    func backButton() {
        ...
        barButton.title = ""
        barButton.tintColor = UIColor(named: "AccentColor")
...
//        btn = UIBarButtonItem(image:UIImage(systemName: "line.horizontal.3"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(btnDidTapOpen))
...
//    }
    ...
    func openDatePicker(formate : String, minDate:Date?, _ completionBlock : ((_ value : String,_ date:Date)->())?)
    {
        ...
        let selectAction = RMAction<UIDatePicker>(title: "Select", style: RMActionStyle.done) { (controller) in
            if let pickerController = controller as? RMDateSelectionViewController
            {
                ...
                if completionBlock != nil {
                    completionBlock!(date, pickerController.datePicker.date)
                }
            }
        }
        ...
        if let actionController = RMDateSelectionViewController(style: style, title: "CargoXpress", message: "Select Date", select: selectAction, andCancel: cancelAction)
        {
            ...
            if let date = minDate {
                actionController.datePicker.minimumDate = date
            }
